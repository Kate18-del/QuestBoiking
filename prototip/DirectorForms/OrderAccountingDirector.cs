...
{
    /// <summary>
    /// Форма для просмотра и анализа заказов с правами директора
    /// Предоставляет расширенные возможности фильтрации и экспорта в Excel
    /// </summary>
    public partial class OrderAccountingDirector : Form
    {
        ...
        /// <summary>
        /// Конструктор формы учета заказов для директора
        /// </summary>
        public OrderAccountingDirector()
        {
            InitializeComponent();
        ...
        private void DisplayCurrentUser()
        {
            if (!string.IsNullOrEmpty(CurrentUser.FIO))
            {
                ...
                {
                    label2.Text = $"директор {CurrentUser.FIO}";
                }
            }
            else
            {
                label2.Text = "директор не авторизован";
            }
        }
        ...
        /// Разрешает только цифры (поиск по номеру заказа)
        /// </summary>
